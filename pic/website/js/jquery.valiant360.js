    ...
    Plugin.prototype = {
        ...
        createMediaPlayer: function() {
            ...
            if( $(this.element).attr('data-photo-src') ) {
                ...
                var vid = $(this.element).attr('data-video-src');
                self._video.src = vid;
                $(self._video).bind("canplaythrough", function () {
                    ...
                    var timer = setInterval(function(){//循环执行
                        ...
                        if(end < duration){
                            //console.dir(end);
                            var wid =  end/duration *100 + '%';
                            $('body').find('.buffer_pro').css('width', wid);
                            return
                        }else if(end == duration){
                            $('body').find('.buffer_pro').css('width', '100%');
                            return
                        }
                        clearInterval(timer);
                    },300)
...
//           xhr.send();
        ...
        createControls: function() {
            ...
            var wid = (this.options.volume*100)+ '%';
            var controlsHTML = ' \
              <div class="controlsWrapper">\
                <div class="valiant-progress-bar">\
                    <div style="width: 0;"></div><div style="width: 100%;"></div><span class="buffer_pro" style="position:absolute;left:0;height:5px;top:0;background:#ccc;"></span>\
                </div>\
                <div class="controls"> \
					...
					<div class="audioControl">\
						...
						<div class="volumeControl">\
							<div class="volumeBar" style="width:'+wid+';">\
                    ...
                    <a href="#" class="fullscreenButton button fa fa-expand"></a><a class="fr open_local_file hide"></a><a class="fr vr_btn"></a>\
                </div> \
              </div>\
            ';
        ...
        attachControlEvents: function() {
            ...
            //点击暂停按钮开始
            $('.video_play_container').on('click','.suspend',function(){
                self.play(); 
            });
            /*点击vr按钮*/
            $(this.element).on('click','.vr_btn',function(){
                  var vrJson ={
                        url:'',
                        time:''
                    };
            ...
            $(this.element).on('dblclick',function(e){
                e.preventDefault();
                var elem = $(self.element)[0];
                if($(this).find('.fullscreenButton').hasClass('fa-expand')) {
                    if (elem.requestFullscreen) {
            ...
            /*$(this.element).on('dblclick',function(e){
                e.preventDefault();
                if($(this).find('.playButton').hasClass('fa-pause')) {
                        $(this).find('.playButton').removeClass('fa-pause').addClass('fa-play');
                        self.pause();
                    } else {
                        $(this).find('.playButton').removeClass('fa-play').addClass('fa-pause');
                        self.play();
                    }
            });*/
            /*$(this.element).on('dblclick',function(e){
        ...
        onProgressClick: function(event) {
            if(this._isVideo && this._video.readyState === this._video.HAVE_ENOUGH_DATA) {
                ...
                console.dir(this._video.buffered.end(0))
            }
        },
		...
		onVolumeMouseMove: function(event){
			...
			if(this._volumeMouseDown){
				...
				var percent =  (this.relativeX - volumeControl.offset().left + $('.valiantPhoto ').offset().left + (volumeControl.find('.volumeBar > .volumeCursor').width()/2)) / volumeControl.width() * 100;
        ...
        play: function() {
            //code to play media
            this._video.play();
            $('.video_play_container .suspend').hide();
            $(this.element).find('.playButton').addClass('fa-pause').removeClass('fa-play');
        ...
        pause: function() {
            //code to stop media
            this._video.pause();
            $('.video_play_container .suspend').show();
            $(this.element).find('.playButton').removeClass('fa-pause').addClass('fa-play');
