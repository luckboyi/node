(function($, PAY) {
    ...
    chinabank.mini_init = function() {
        ...
        $.each(PAY.bankConfig, function(index, val) {
            if (val.chinabank_id) {
                ...
                buttonhtml += '<span bank-id="' + val.chinabank_id + '" class="choice-bank-list"><i></i><span class="bank-icon-' + index + ' bank-icon"></span></span>';
            }
        });
        buttonhtml += "</div>";
    ...
    chinabank.web_init = function() {
        ...
        buttonhtml += '<input type="button" class="pay-button" value="网银充值">';
        $("#button-container").html(buttonhtml);
    ...
    //根据不同选择内容进行不同支付操作
    chinabank.checkpay = function() {
        var _this = this;

        $("#hideParam input[name='channel']").val("chinabank");
    ...
    chinabank.gopay = function() {
        ...
        var url = urlconfig.pay + "/index.php?action=" + PAY.channels.chinabank.action.form + "&resource_id=1282160";
