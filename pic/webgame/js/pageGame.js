...
function resizeFn(reset) {
    ...
    var tinyscrollcon = [
        "pageGame_list_scrollbar",
        "pageGame_con_scrollbar",
        "pageGame_left_list_scrollbar"
    ];

    if (tinyscrollbox.length == 0 || reset == true) {
        $(tinyscrollcon).each(function(i, e) {
            var tinyscrollobj = $("#" + e).tinyscrollbar();
            tinyscrollbox.push(tinyscrollobj.data("plugin_tinyscrollbar"));
        });
    } else {
        $(tinyscrollbox).each(function(i, e) {
            if (typeof e != "object") {
                var tinyscrollobj = $("#" + tinyscrollcon[i]).tinyscrollbar();
                e = tinyscrollobj.data("plugin_tinyscrollbar");
                if (typeof e == "object") {
                    tinyscrollbox[i] = e;
                    e.update("relative");
                }
