...
function loginInFn(){
    ...
    $('#registerForm input[name="phoneNumber"]').blur(function(){
        ...
        PL.callFun('loginframe', 'checkaccount', JSON.stringify(json));
    });
    //当密码错的时候
    ...
    $('#send_btn').click(function(){
        ...
        //$(this).value="免费获取验证码";
        PL.callFun('loginframe', 'req_smscode', phoneNum);
    ...
    $('.verification_sure ').on('click',function(){
        ...
        PL.callFun('loginframe', 'verify_res', verificationNum);
    });
    //点击刷新
    ...
    $('.in_login_input ul').on('click','li',function(){
        ...
        $('.in_login_input ul').hide();
        $('.pull-down').removeClass('selected');
        PL.callFun('loginframe', 'select_item', phoneNum);
    });
    //点击关闭提示
    $('.in_login_input ul ').on('click','.input_close',function(e){
        ...
        PL.callFun('loginframe', 'delete_item',''+account+'');
    ...
    function loginFn(){
        ...
        //调用数据
        PL.callFun('loginframe', 'login',JSON.stringify(login_in));
    }
    //按enter键
    $(window).on('keydown',function(e){
        if(e.which == 13){
            ...
            PL.callFun('loginframe', 'checkimgcode', '{"imgcode":"'+val+'"}');
        }
    });
    ...
    $('.other_enter').on('click','span',function(){
        ...
        if($(this).hasClass('qq_enter')){
            ...
            /*$('body').append(html);*/
            PL.callFun('loginframe', 'qqlogin', 'http://passport.vronline.com/auth/qq?url=vr_windows');
        }else if($(this).hasClass('wx_enter')){
            PL.callFun('loginframe', 'wxlogin', 'http://passport.vronline.com/auth/wx?url=vr_windows');
        }else if($(this).hasClass('wb_enter')){
            PL.callFun('loginframe', 'wblogin', 'http://passport.vronline.com/auth/weibo?url=vr_windows');
        }else if($(this).hasClass('3d_enter')){
            PL.callFun('loginframe', 'bblogin', 'http://passport.vronline.com/auth/3Dbobo?url=vr_windows');
        }
    });
    //点击关闭按钮
    ...
    $('.login_sel ').on('click',function(){
        ...
        if($(this).hasClass('selected')){
          $(this).parents('.login_agreement').find('.agreement').removeClass('erroColor')
          $(this).parents('.login_agreement').find('.agreement a').removeClass('erroColor')
        }else {
          $(this).parents('.login_agreement').find('.agreement').addClass('erroColor')
          $(this).parents('.login_agreement').find('.agreement a').addClass('erroColor')
    ...
    function registerFn(){
        ...
        if($('.login_agreement i.login_sel').hasClass('selected')){
            //alert('请认真阅读相关协议，并勾选');
            PL.callFun('loginframe', 'register',JSON.stringify(register_msg));
        }
    }
    //按enter键
    $(window).on('keydown',function(e){
    ...
    $('.offline_login .sure').on('click',function(){
        PL.callFun('loginframe', 'offlineloginconfirm', '');
    })
}
function showLogin(){
    $('.login_enter_erro').hide();
