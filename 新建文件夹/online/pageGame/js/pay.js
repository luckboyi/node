    ...
    $.ajax({
        ...
        success:function(data){
            if(data.code != 0){
                ...
                if(pay_method == 'ewm'){
                    payCallBack("http://dev3.pay.xy.com/index.php?action=alipayscanvr&resource_id=1282160");
...
function payAlipay(pay_way,json,pay_method){
    switch (pay_way) {
        case 0:
            if(pay_method == 'ewm'){
                ajaxFn('http://pay.vronline.com/create/buyplantb',json,'ewm')
            }else{
                ajaxFn('http://pay.vronline.com/create/buyplantb',json,'pay_name')
            }

            break;
        case 1:
            if(pay_method == 'ewm'){
                ajaxFn('http://pay.vronline.com/create/buygame',json,'ewm')
            }else{
                ajaxFn('http://pay.vronline.com/create/buygame',json,'pay_name')
            }
            break;
        case 2:
