   ...
   tipsFn.config ={
    ...
    tipsFn.container =function(){
        ...
        containerHtml +='<div class="tips_con" style="padding: 12px; background:#1c202a; border: 1px solid #000; color: #828f9e;position: absolute;left: 50%;top:50%; transform: translate(-50%,-50%);"><div class="tips_header"></div><div class="tips_msg"  style="padding: 20px 100px; background: #151920;margin: 10px 0;"></div><div class="tips_btn" style="height: 24px;"><ul class="clearfix" style="display: inline-block; ">' +
            ...
            '<li class="fl cancel tips_botton" id="tips_btn" style="cursor:default;margin:0 5px;line-height: 24px;width: 80px;text-align: center; border: 1px solid #000; background:#384455 ;">取消</li></ul></div></div></div>';
    ...
    tipsFn.modifyCon = function(modifyMsg,phonemsg,errmsg){
        ...
        $('.get_code').on('click',function(){
            ...
            $.ajax({
                    ...
                    type:'GET',
                    dataType:'json',
                    data:{mobile : phonemsg,action : "mobileChange"},
        ...
        $('.get_code').on('click',function(){
            ...
            if (changeMobile.length == 0) {
                ...
                $.ajax({
                    ...
                    type:'GET',
                    dataType:'json',
                    data:{mobile : changeMobile,action : "mobileChange"},
                    headers: {  // header属性，是为了避免跨站伪造请求攻击写的
                        'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
                    },
                    success: function(data){
...
function verifyBindCode(mobile,code,obj){
    ...
    $.ajax({
        ...
        type:'GET',
        dataType:'json',
        data:{mobile : phoneNum,code : code,action : "1"},
        headers: {  // header属性，是为了避免跨站伪造请求攻击写的
            'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
        },
        success: function(data){
...
function sendMobileMsgOpen(mobile){
	 $.ajax({
		...
		type:'GET',
		dataType:'json',
		data:{mobile : mobile, action : "mobileChange"},
		headers: {  // header属性，是为了避免跨站伪造请求攻击写的
			'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
		},
		success: function(data){
...
function verifyBindCodeOpen(mobile,code,obj){
    var phoneNum = mobile;
    var code = code;
    var _this = obj;
    $.ajax({
        ...
        type:'GET',
        dataType:'json',
        data:{mobile : phoneNum,code : code,action : ""},
        headers: {  // header属性，是为了避免跨站伪造请求攻击写的
            'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
        },
        success: function(data){
...
function changeBindMobile(mobile,code,obj,newMobile){
    ...
    if (mobile.length > 10 && mobile.length < 12 && code.length > 5) {
        ...
        type:'GET',
        dataType:'json',
        data:{mobile : mobile,code : code,action : newMobile},
        headers: {  // header属性，是为了避免跨站伪造请求攻击写的
            'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
        },
        success: function(data){
